...
const callerNumber = config.TWILIO_VOICE_CALLER_NUMBER;
 ...
 */
export const tokenGenerator = (request, response) => {
  ...
  const apiKey = config.TWILIO_VOICE_API_KEY;
  const apiSecret = config.TWILIO_VOICE_API_KEY_SECRET;

  // Used specifically for creating Voice tokens
  const pushCredSid = config.TWILIO_VOICE_PUSH_CREDENTIAL_SID;
  const outgoingApplicationSid = config.TWILIO_VOICE_APP_SID;
 ...
 */
export const makeCall = (request, response) => {
 ...
 */
export const placeCall = async(request, response) => {
  // The recipient of the call, a phone number or a client
  var to = null;
  if (request.method === 'POST') {
    to = request.body.to;
